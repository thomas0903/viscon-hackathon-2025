...
def _local_path_for_profile_url(url: str | None) -> Path | None:
    if not url:
        return None
    s = str(url)
    if s.startswith("/uploads/user/"):
        rel = s.lstrip("/")  # uploads/user/...
        return Path("var") / Path(rel)
    return None


def _ensure_avatar_for_user(user: dict) -> bool:
    """Ensure user has a thumbnail file; returns True if user was modified.

    Skips network fetch if the file already exists locally.
    """
    modified = False
    uid = str(user.get("id") or user.get("username") or "").strip()
    if not uid:
        return modified
    # If a URL is present and the local file exists, nothing to do
    lp = _local_path_for_profile_url(user.get("profile_picture_url"))
    if lp is not None and lp.exists():
        return modified
    # If a local avatar already exists in the user folder, reuse it
    dest = _avatars_root() / uid
    if dest.exists():
        for cand in ["avatar.jpg", "avatar.png", "avatar.webp", "avatar.jpeg"]:
            if (dest / cand).exists():
                user["profile_picture_url"] = f"/uploads/user/{uid}/{cand}"
                return True
    # Otherwise, try to fetch and write avatar
    try:
        seed = hashlib.sha1((uid).encode("utf-8")).hexdigest()
        url = f"https://i.pravatar.cc/256?u={seed}"
        fname = _download_avatar(url, dest)
        if fname:
            user["profile_picture_url"] = f"/uploads/user/{uid}/{fname}"
            modified = True
    except Exception:
        pass
    return modified
...
def make_users(n):
    ...
    for _ in range(n):
        ...
        while cand in seen:
            ...
            suf += 1
        seen.add(cand)
        vis = random.choice(["all", "friends", "ghost"])  # VisibilityMode
        user = {
            "id": cand,
            "username": cand,
            "first_name": first,
        ...
        # Try to assign a thumbnail avatar via pravatar
        try:
            seed = hashlib.sha1((user["username"]).encode("utf-8")).hexdigest()
            url = f"https://i.pravatar.cc/256?u={seed}"
            dest = _avatars_root() / user["id"]
            fname = _download_avatar(url, dest)
            if fname:
...
# -------------------
def increase_assortativity_connected(G, target=ASSORTATIVITY_TARGET, C_floor=None,
                                     max_iters=REWIRE_MAX_ITERS, batch=REWIRE_BATCH, seed=SEED):
    """
    ...
    for _ in range(max_iters):
        H2 = H.copy()
        try:
            swapped = nx.connected_double_edge_swap(H2, nswap=batch, _window_threshold=3,
...
# Preview
# -------------------
def preview_graph(G, node_to_uid, html_out="friends.html"):
    for n in G.nodes():
        G.nodes[n]["label"] = node_to_uid[n]
        G.nodes[n]["title"] = f"{node_to_uid[n]}"
        G.nodes[n]["value"] = G.degree[n]
    if HAS_PYVIS:
        nt = Network(height="750px", width="100%", bgcolor="#0b0b0b", font_color="#f0f0f0")
        nt.barnes_hut()
        nt.from_nx(G)
        nt.show(html_out, notebook=False)
    elif HAS_MPL:
        pos = nx.spring_layout(G, seed=SEED)
        labels = {n: node_to_uid[n] for n in G.nodes()}
        sizes = [200 + 30 * G.degree[n] for n in G.nodes()]
        nx.draw_networkx(
            G, pos=pos, labels=labels, node_size=sizes, width=0.6,
            edge_color="#999999", node_color="#66b3ff", font_size=7
        )
...
# -------------------
def main():
    # 0) Try to load cached users; if present, reuse and ensure avatars
    cached_users = _load_cached_users()
    users_modified = False
    if cached_users is not None:
        for u in cached_users:
            if _ensure_avatar_for_user(u):
                users_modified = True
        users = cached_users
        n_users = len(users)
    else:
    ...
    if users is None:
        ...
        # Ensure avatars for newly created users
        for u in users:
            if _ensure_avatar_for_user(u):
                users_modified = True
    ...
    # 6) Preview rewired graph
    preview_graph(G, node_to_uid, html_out="friends.html")
